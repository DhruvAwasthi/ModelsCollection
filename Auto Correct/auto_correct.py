...
def process_data(file_name):
    ...
    words = re.findall(r'\w+', read_data)
    ### END CODE HERE ###
...
def get_probs(word_count_dict):
    ...
    probs = {w: word_count_dict[w] / m  for w in word_count_dict}
    ### END CODE HERE ###
    return probs
...
def replace_letter(word, verbose=False):
    ...
    ### START CODE HERE ###
    replace_set = set([word.replace(i, c) for i in word for c in letters])
    ### END CODE HERE ###
...
def edit_one_letter(word, allow_switches = True):
    ...
    if allow_switches: edit_one_set.update(set(switch_letter(word)))
    ### END CODE HERE ###
...
def edit_two_letters(word, allow_switches = True):
    ...
    for l in tmp_edit_one_set:
        edit_two_set.update(set(edit_one_letter(l, allow_switches)))
    ### END CODE HERE ###
...
def get_corrections(word, probs, vocab, n=2, verbose = False):
    ...
    if word in vocab:
        temp.append(word)
    one_letter_suggestion = sorted(list(edit_one_letter(word)))
    for i in one_letter_suggestion:
        if i in vocab:
            temp.append(i)
    two_letter_suggestion = sorted(list(edit_two_letters(word)))
    for i in two_letter_suggestion:
        if i in vocab:
            temp.append(i)
    temp.append(word)
    for i in range(n):
        suggestions.append(temp[i])
        n_best.append((temp[i], probs[temp[i]]))
    ### END CODE HERE ###
